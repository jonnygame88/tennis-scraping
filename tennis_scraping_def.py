...
def player_ref_data(player_id, gender, yr_start, yr_end):
    ...
    ['m_no', 'player_id', 'opponent_id', 'date', 'won', 
    'completed', 'sets', 'match_url', 'event_url'])
    ...
    for yr_url in yr_links:
        ...
        for i in range(len(opp_ids)):
            player_df.loc[r+i] = ['m'+str(r+i), player_id, opp_ids[i], 
                          dates[i], ref_win[i], completed[i], sets[i][0], 
...
def get_name(player_id, gender):
    url0 = 'http://www.tennisbetsite.com/'
    url1 = 'index.php?option=com_mvx_trdb&type=playerinfo&playerid='
    url2 = '&association=A&Itemid=29/' if gender == 'm' else '&association=W&Itemid=29'
    s = soup(url0 + url1 + str(player_id) + url2)
