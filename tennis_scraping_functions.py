...
def match_ref(pyr_soup):
    ...
    return([x.replace(' ', '') for x in info if len(x) > 0 and x != 'Surf.'])
...
def match_stats(match_soup):
    ...
    if gs is None:
        ...
        return([i.get_text() for i in gs.findAll(attrs = {'class': 'value'})])
    
## get match stats as data frame
def match_stats_df(match_soup):
    gs = match_soup.findAll('table', attrs = {'class': 'gamestats'})[0].tbody
    if gs is None:
        return([])
    else:
