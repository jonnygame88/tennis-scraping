...
elos = [[0.01, 0.01, 0.01, 0.01, 0.01, 0.01, 0.01] for i in range(N)]
defaults = [1500, 1750, 2000, 2000, 2000, 2000, 1750]
mcounts = [[0, 0, 0, 0, 0, 0, 0, 0] for i in range(N)]
mlast = [0 for i in range(N)]
...
for i in range(len(elo_input)):
    ...
    if ntotal1 == 0: elo1 = defaults[level-1]
    else: elo1 = elos[p1][0]
    if ntotal2 == 0: elo2 = defaults[level-1]
    else: elo2 = elos[p2][0]
    ...
    if elo_input.completed.loc[i] == 1 and level <= 5:
